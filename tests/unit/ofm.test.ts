...
describe("Obsidian comments", () => {
  ...
  it("leaves an unclosed comment marker as literal text", async () => {
    const html = await renderMarkdown("%%open", {}, "source.md");
...
describe("Obsidian highlights", () => {
  ...
  it("leaves an unclosed highlight marker as literal text", async () => {
    const html = await renderMarkdown("==open", {}, "source.md");
...
describe("math / LaTeX", () => {
  ...
  it("leaves unpaired dollar signs as literal text", async () => {
    const html = await renderMarkdown("The price was $5.", {}, "source.md");

    expect(html).toContain("The price was $5.");
    expect(html).not.toContain('<span class="katex">');
  });
