...
compile-time constants in `src/site.ts`, and the only build-time knob is the
`MUFFIN_BASE_PATH` environment variable (used for GitHub Pages sub-path
deployments):
...
```ts
export const SITE = { title: "Muffin", lang: "en" };
...
Change those constants to point at your vault and site; the GitHub Pages
workflow (`deploy.yaml`) sets `MUFFIN_BASE_PATH: /Muffin` so all generated URLs
are prefixed for a sub-path deployment.
...
```
(compile-time constants in src/site.ts + MUFFIN_BASE_PATH env)
content/*.md
...
Pipeline order matters: site constants and the `MUFFIN_BASE_PATH` env are the
only inputs. `withBasePath` reads the env at call time, so every generated URL
(wikilink hrefs, asset embeds, explorer links, styles.css/KaTeX hrefs, and the
`<body data-base-path>` scoping hook) picks it up uniformly. `buildSiteGraph`
...
Template tokens: `{{TITLE}}`, `{{CONTENT}}`, `{{TOC}}`, `{{BACKLINKS}}`, `{{NAV}}`, `{{CSS}}`, `{{KATEX_CSS}}`, `{{PAGE_META}}`, `{{SITE_TITLE}}`, `{{LANG}}`, `{{SITE_DESCRIPTION}}`, `{{BODY_ATTRS}}`.